...
import admin.purchase;
import com.formdev.flatlaf.IntelliJTheme;
import com.loay.connection.loginData;
...
public class Main extends javax.swing.JFrame {
   ...
   private void register() throws SQLException {
    ...
    if (serviceUser.checkDuplicateUser(user.getUserName())) {
        showMessage(Message.MessageType.ERROR, "User name already exists");
        return;
    }
    // Check for duplicate email
    if (serviceUser.checkDuplicateEmail(user.getEmail())) {
    ...
    ModelLogin user = loginAndRegister.getDataLogin();
    ...
    loginData dbManager = new loginData();
    DashBoard dashboard = new DashBoard();
    purchase Purchase = new purchase();
    

    dbManager.connect(); // Establish database connection
    if (!dbManager.isValidLogin(user.getEmail(), user.getPassword())) {
        showMessage(Message.MessageType.ERROR, "Invalid email or password");
    } else {
        showMessage(Message.MessageType.SUCCESS, "Login successful");
        // Additional actions after successful login, e.g., redirect to home page
        
        // Check if the user is an admin and open the dashboard
...
Purchase  .setVisible(true);             
    ...
    dbManager.disconnect(); // Close database connection
}
